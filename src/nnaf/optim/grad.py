...
class Deriviator:
    ...
    def __call__(
        ...
        **flows: dict[str, list[str]],
        ...
        identities, strflows = zip(*sorted(flows, key=lambda x: len(x[1]), reverse=False))
        for identity, strflow in zip(identities, strflows):
        ...
        for node in LevelOrderIter(self.root, filter_=lambda n: n.depth > 0):
            node.grad = torch.autograd.grad(node.parent.grad, self.vars[node.var], retain_graph=True, create_graph=True)[0]
